...
export const QuestionStoreModel = types
    ...
    .actions((self) => ({
        saveQuestions: (questionSnapshots: QuestionSnapshot[]) => {
            const questionModels: Question[] = questionSnapshots.map((c) =>
                QuestionModel.create(c)
    ...
    .actions((self) => ({
        getQuestions: flow(function* () {
            ...
            if (result.kind === 'ok') {
                ...
                __DEV__ && console.error(result.kind);
            }
        }),
...
export const createQuestionStoreDefaultModel = () =>
    types.optional(QuestionStoreModel, {});
